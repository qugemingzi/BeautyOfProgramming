public class MS_1 {
    ...
    public static int Win_The_War(int n, int k, int[] arr){
        ...
        int[] counts = new int[max - min + 1];      // 通过min, max缩小counts大小
        int[][] group = new int[nums][k];           // 分组表
        for(int temp : arr) counts[temp - min]++;
        int score = 0, complete = nums, colpos = 0, rowpos;
        for(int temp : arr) {
            if (counts[temp - min] > nums)
                return -1;                          // 某个数比分组数还多，无法分组
        }
        while(complete > 0 && colpos < k) {
            for(int i = 0; i <= max - min; ++i){    // 将剩余能量值中个数与剩余组数相等的均分每组
            ...
            rowpos = colpos;                        // 行填完数
            for(int i = 0; i <= max - min && rowpos < k; ++i){  // 依次放入分组中
                if(counts[i] > 0){
